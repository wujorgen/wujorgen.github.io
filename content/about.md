+++
date = '2026-06-08T23:04:22-04:00'
draft = false
title = 'About'
+++


I am currently a engineer at the Naval Nuclear Laboratory, where I work on thermal hydraulics.
I am experienced with high-performance computing in Linux environemnts, computational fluid dynamics, and scientific machine learning.

I am also studying at the University of Pittsburgh, where I am working on my PhD under the supervision of Drs. Peyman Givi and Masoud Barati.
My research interests are quantum computing, machine learning, and optimization.

Outside of my professional and academic life, I am into photography, music production, and working on cars. My photography work can be found [here](https://www.riverwatchdigital.com).

### Technical Profile

* **Languages:** Python, Julia, Fortran, C/C++
* **Parallel Computing:** Shared-Memory Multiprocessing (OpenMP), Multi-threading, GPU Acceleration (CUDA)
* **Modeling Paradigms:** Scientific Machine Learning (SciML), Reduced-Order Modeling (ROM)
* **Domain Focus:** Computational Fluid Dynamics (CFD), Numerical Methods
* **Toolchain & Environment:** Linux, Git, CMake

### Let's Connect

I am always open to discussing scientific computing, fluid dynamics, or software engineering opportunities.

* 📥 <a href="/JW_ATS_resume_forgradschool.pdf" target="_blank" rel="noopener noreferrer">Download My Resume</a>
* 💼 [Connect on LinkedIn](https://linkedin.com/in/jorgen-wu)
* 💻 [GitHub](https://github.com/wujorgen/)
* ✉️ [Email Me](mailto:wujorgen@gmail.com)
